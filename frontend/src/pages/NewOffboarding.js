...
export default function NewOffboarding() {
  ...
  const generatePreview = () => {
    ...
    const exitDate = formData.start_date;
    const tasks = selectedTemplate.tasks.map(t => {
      const dueDate = new Date(exitDate);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Offboarding erfolgreich erstellt!");
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6" data-testid="new-offboarding">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight flex items-center gap-3">
            <UserMinus className="w-8 h-8 text-purple-600" />
            Neues Offboarding
          </h1>
      ...
      <div className="flex gap-2">
        <div className={`flex-1 h-2 rounded-full ${step >= 1 ? 'bg-purple-600' : 'bg-slate-200'}`} />
        <div className={`flex-1 h-2 rounded-full ${step >= 2 ? 'bg-purple-600' : 'bg-slate-200'}`} />
      </div>
      ...
      {step === 1 && (
        <div className="space-y-6">
          ...
          {employees.length > 0 && !selectedEmployee && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Wählen Sie einen bestehenden Mitarbeiter für das Offboarding</CardDescription>
              </CardHeader>
                ...
                <div className="max-h-60 overflow-y-auto space-y-2">
                  {filteredEmployees.map(emp => (
                    <div
                      ...
                      data-testid={`employee-${emp.employee_email}`}
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant={emp.status === "completed" ? "secondary" : "outline"}>
                          {emp.status === "completed" ? "Onboarding abgeschlossen" : "Aktiv"}
                        </Badge>
                      </div>
                    </div>
                  ))}
                  {filteredEmployees.length === 0 && (
                    <p className="text-center text-slate-500 py-4">Keine Mitarbeiter gefunden</p>
                  )}
                </div>
              </CardContent>
            </Card>
          ...
          {/* Manual entry or selected employee */}
          <Card>
            <CardHeader>
              <CardTitle>{selectedEmployee ? "Ausgewählter Mitarbeiter" : "Manuelle Eingabe"}</CardTitle>
              <CardDescription>
                {selectedEmployee 
                  ? "Überprüfen und ergänzen Sie die Daten"
                  : "Oder geben Sie die Daten manuell ein"}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {selectedEmployee && (
                <Button variant="outline" size="sm" onClick={() => {
                  ...
                  setFormData({ ...formData, employee_name: "", employee_email: "", location: "", manager_email: "", linked_case_id: null });
                }}>
                ...
                <div className="space-y-2">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant="outline" className="w-full justify-start text-left font-normal" data-testid="exit-date-picker">
              ...
              <div className="flex justify-end pt-4">
                <Button onClick={generatePreview} disabled={!isStep1Valid} className="bg-purple-600 hover:bg-purple-700 text-white" data-testid="next-step">
      ...
      {step === 2 && preview && (
        <Card>
          <CardHeader>
            <CardTitle>Vorschau der Offboarding-Tasks</CardTitle>
            <CardDescription>
              {preview.length} Tasks werden für das Offboarding von {formData.employee_name} erstellt
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            {/* Warnings */}
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-amber-700 mt-1">
                    Tasks mit negativem Offset werden vor dem Austrittsdatum fällig. Bitte prüfen Sie das Datum.
                  </p>
                </div>
              </div>
            )}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {task.evidence_required && (
                          <Badge variant="outline" className="text-xs text-purple-600 border-purple-300">Nachweis erforderlich</Badge>
                        )}
                      </div>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-slate-400">
                        {task.offset_days === 0 ? "Am Austrittsdatum" : task.offset_days > 0 ? `+${task.offset_days} Tage` : `${task.offset_days} Tage`}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex justify-between pt-4">
              ...
              <Button onClick={handleSubmit} disabled={loading} className="bg-purple-600 hover:bg-purple-700 text-white" data-testid="create-offboarding">
                {loading ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
                  <>
                    <CheckCircle2 className="w-4 h-4 mr-2" /> Offboarding erstellen
                  </>
                )}
              </Button>
            </div>
